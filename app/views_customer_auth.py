...
def customer_register(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # ✅ EMAIL MANDATORY
            if not email:
                messages.error(request, '❌ Email zaroori hai (OTP verification ke liye)')
                return redirect('customer_register')
...
def send_order_otp(request):
    ...
    if not customer.email:
        return JsonResponse({
            'success': False,
            'message': 'Email required! Please add email to your account first.',
            'requires_email': True,
        })
    
    # Trusted customer skip
    ...
    try:
        ...
        if otp.is_sent_to_customer:
            ...
            return JsonResponse({
                'success': True,
                'skip_otp': False,
                'message': f'⚠️ OTP generated but email failed. Contact admin.',
                'otp_id': otp.id,
                'order_token': order_token,
                'email_sent': False,
            })
