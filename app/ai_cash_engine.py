...
class AICashEngine:
    ...
    def __init__(self):
        ...
        from .models import (
            ...
            SaleInstallment, EmiPayment, EmergencyFund
        )
    ...
    def calculate_health_score(self):
        ...
        # ✅ FIXED: Decimal aur Float mixing issue
        # ==========================================
        current_balance = Decimal(str(self.CashBalance.get_balance()))
        last_30 = self.today - timedelta(days=30)
        ...
        # ✅ FIX: Pehle float mein convert karo
        current_balance_float = float(current_balance)
        monthly_expenses_float = float(monthly_expenses)
        liquidity_days = (current_balance_float / monthly_expenses_float * 30) if monthly_expenses_float > 0 else 999
        ...
        # ✅ UPDATED: Alag Emergency Fund check karo
        # ✅ FIXED: Decimal aur Float mixing issue
        # ==========================================
        avg_monthly_expense = float(monthly_expenses)
        emergency_fund_target = avg_monthly_expense * 3  # 3 mahine ka
        
        # ✅ Alag Emergency Fund check karo
        emergency_fund = self.EmergencyFund.objects.filter(is_active=True).first()
        
        if emergency_fund:
            # Alag fund use karo
            buffer_amount = float(emergency_fund.current_amount)
            buffer_months = emergency_fund.months_covered
            buffer_source = 'separate_fund'
        else:
            # Total cash use karo (default)
            buffer_amount = float(current_balance)
            # ✅ FIX: Pehle float mein convert karo
            buffer_months = (current_balance_float / avg_monthly_expense) if avg_monthly_expense > 0 else 0
            buffer_source = 'total_cash'
        
        if emergency_fund_target > 0:
            buffer_ratio = buffer_amount / emergency_fund_target
        ...
        return {
            ...
            'factors': {
                ...
                'buffer': {
                    ...
                    'value': f'{buffer_months:.1f} mahine' if buffer_months > 0 else '0 mahine',
                    'description': 'Bure waqt ke liye bachat',
                    'source': buffer_source,
                    'current_amount': buffer_amount,
                },
                'inflow': {
                    'score': inflow_score,
                    'max': 20,
                    ...
                    'description': 'Money received (last 30 days)',
                },
                'outflow': {
                    'score': outflow_score,
                    'max': 15,
                    ...
                    'description': 'Money spent (last 30 days)',
                },
                'predictability': {
